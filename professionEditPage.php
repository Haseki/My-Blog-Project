...
$result = selectDBforAdmin("*", "professions", "id = $id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($isFileSet){
        $result2 = updateDB("professions" , "title = \"$title\", summary = \"$sum\", img_name = \"$filename\", img_src = \"$img_src\", teori = \"$teori\", pratik = \"$pratik\"", "id = $id");
    }else{
        $result2 = updateDB("professions" , "title = \"$title\", summary = \"$sum\", teori = \"$teori\", pratik = \"$pratik\"", "id = $id");
    }
...
<div class="article_blue_form" style="height: auto; padding-bottom: 100px;">
    <form method="POST" action="<?php echo './professionEditPage.php?id='.$id; ?>" enctype="multipart/form-data">
            <br><br>
